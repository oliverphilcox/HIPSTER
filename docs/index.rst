...
To compute a bispectrum up to :math:`\ell=L` from a particles in a simulation box (``data.dat``), with pair counts truncated at radius ``R0`` with :math:`k`-space binning file ``binning.csv`` on 4 CPU-cores, we simply run::
