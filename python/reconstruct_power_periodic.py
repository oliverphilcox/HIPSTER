...
if len(sys.argv)<5:
    print("Please specify input parameters in the form {DD_FILE} {GAL_FILE} {OUTFILE}")
    sys.exit()

DD_input = str(sys.argv[1])
gal_file = str(sys.argv[2])
...
if periodic:
    norm = 0.
    gal_x,gal_y,gal_z = np.loadtxt(gal_file)[:,:3]
...
raise Exception('Add RR correction')
